...
This applies only to command _objects_. If two identical commands are scheduled - even if they both implement Java's ``equals()`` method - the scheduler still treats them as different commands, and the second command will interrupt and cancel the first:
