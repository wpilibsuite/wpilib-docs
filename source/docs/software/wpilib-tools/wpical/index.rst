...
WPIcal uses an "ideal" field map JSON as an initial guess point for the optimization. It is recommended to upload the json file included with WPILib, which can be found here: `Field JSON <https://github.com/wpilibsuite/allwpilib/tree/v2027.0.0-alpha-7/apriltag/src/main/native/resources/org/wpilib/vision/apriltag>`_. If using a custom map, it must follow these rules:
